...
class RAGQueryRequest(BaseModel):
    ...
    query: str = Field(
        ..., min_length=1, max_length=4000, description="The user's natural language query"
    )
    strategy: str = Field(
        default="auto",
    ...
    filters: dict[str, Any] = Field(
        default_factory=dict, description="Reserved for future use. Must be empty for now."
    )
    top_k: int = Field(
        default=5, ge=1, le=20, description="Reserved for future use. Must be 5 for now."
    )
    ...
    @classmethod
    def validate_filters_not_supported(cls, v: dict[str, Any]) -> dict[str, Any]:
        if v:
            raise ValueError("filters are not yet supported for /rag/query; omit this field")
        return v

    @field_validator("top_k")
    @classmethod
    def validate_top_k_not_supported(cls, v: int) -> int:
        if v != 5:
            raise ValueError("top_k customization is not yet supported for /rag/query; use top_k=5")
        return v
...
class RAGStrategyEvaluateRequest(BaseModel):
    ...
    query: str = Field(..., min_length=1, max_length=4000, description="The test query to evaluate")
    ground_truth_relevant_ids: list[str] = Field(
        default_factory=list, description="List of expected relevant chunk IDs"
    )
    ground_truth_answer: str | None = Field(
        default=None, description="Expected answer text for generation evaluation"
    )
    iterations: int = Field(
        default=1, ge=1, le=10, description="Number of iterations to run for averaging"
    )
    context: dict[str, Any] | None = Field(
...
class RAGBenchmarkRequest(BaseModel):
    ...
    benchmark_name: str = Field(
        ..., min_length=1, description="Name of benchmark dataset (ms_marco, custom_qa, etc.)"
    )
    max_queries: int = Field(
        default=100, ge=1, le=1000, description="Maximum number of queries to evaluate"
    )
    k_values: list[int] = Field(
        default_factory=lambda: [5, 10], description="K values for retrieval metrics"
    )
...
class RAGMetricsSummary(BaseModel):
    ...
    strategy_usage: dict[str, int] = Field(
        default_factory=dict, description="Usage count per strategy"
    )
    query_type_distribution: dict[str, int] = Field(
        default_factory=dict, description="Distribution of query types"
    )
...
class RAGMetricsResponse(BaseModel):
    ...
    component_health: dict[str, Any] = Field(
        default_factory=dict, description="Health status of RAG components"
    )
    recent_alerts: list[dict[str, Any]] = Field(
        default_factory=list, description="Recent evaluation alerts"
    )
