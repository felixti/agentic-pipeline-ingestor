...
class BulkIngestRequest(BaseModel):
    ...
    default_pipeline_id: Optional[UUID] = Field(default=None)
...
class BulkRetryFilter(BaseModel):
    ...
    job_ids: Optional[List[UUID]] = Field(default=None)
    status: Optional[List[JobStatus]] = Field(default=None)
    ...
    external_ids: Optional[List[str]] = Field(default=None)


class BulkRetryRequest(BaseModel):
    """Request for bulk retry operation."""
    filter: Optional[BulkRetryFilter] = Field(default=None)
    job_ids: Optional[List[UUID]] = Field(default=None)
    force_parser: Optional[str] = Field(default=None)
    updated_config: Optional[Dict[str, Any]] = Field(default=None)
...
async def list_bulk_operations(
    operation_type: Optional[str] = Query(None, pattern="^(ingest|retry|export)$"),
