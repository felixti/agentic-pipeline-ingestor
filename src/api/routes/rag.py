...
from src.rag.models import RAGResult, Source
from src.rag.router import AgenticRAG, STRATEGY_PRESETS
...
def _get_agentic_rag() -> AgenticRAG:
    ...
    Returns:
        AgenticRAG instance
    """
    global _agentic_rag, _query_rewriter, _query_classifier
    ...
    if _agentic_rag is None:
        ...
        _agentic_rag = AgenticRAG(
            ...
            # HyDE and reranker are optional - initialized as None for now
            hyde_rewriter=None,
            reranker=None,
        )
...
async def list_strategies(request: Request) -> RAGStrategiesResponse:
    ...
    strategies.insert(0, RAGStrategyInfo(
        name="auto",
    ...
    try:
        ...
        for iteration in range(eval_request.iterations):
            ...
            if eval_request.ground_truth_relevant_ids:
                ...
                retrieval_results = [
                    type('RetrievalResult', (), {'id': s.chunk_id})()
        ...
        query_type = all_results[0].query_type if all_results else "factual"
        ...
        raise HTTPException(
            ...
            detail={
                "error": {
                    ...
                    "message": f"Failed to evaluate strategy: {str(e)}",
                }
            },
        ) from e
...
async def run_benchmark(
    ...
    try:
        ...
        benchmark_result = await evaluator.run_benchmark(
            ...
            rag_system=rag_system,  # type: ignore[arg-type]
            config=config,
