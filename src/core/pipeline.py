        ...
        if strategy == "fixed":
            ...
            for paragraph in paragraphs:
                ...
                if current_size + paragraph_size > chunk_size and current_chunk:
                    ...
                    current_size = (
                        len(overlap_text) + paragraph_size if overlap_text else paragraph_size
                    )
                    start_char += (
                        len(chunk_text) - len(overlap_text) if overlap_text else len(chunk_text)
                    )
                else:
    ...
    async def execute(self, context: JobContext) -> JobContext:
        ...
        try:
            ...
            for chunk_data, embedding_result in zip(chunks, embedding_results):
                chunk_quality = quality_assessor.text_analyzer.analyze(chunk_data["content"])
            ...
            context.set_stage_result(
                self.name,
                {
                    ...
                    "avg_latency_ms": sum(r.latency_ms or 0 for r in embedding_results)
