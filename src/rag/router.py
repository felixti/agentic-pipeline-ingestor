...
class AgenticRAG:
    ...
    async def process(
        ...
        try:
            ...
            rewrite_result, rewrite_time = await self._rewrite_query(
            ...
            if config.reranking and self.reranker is not None and chunks:
                chunks, reranking_time = await self._rerank_documents(
                    query, chunks, top_k=5
                )
            ...
            logger.info(
                f"RAG pipeline completed in {total_latency_ms:.2f}ms "
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        result["healthy"] = (
            result["components"].get("query_rewriter", {}).get("healthy", False)
