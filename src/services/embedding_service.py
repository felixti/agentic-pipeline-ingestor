...
try:
    ...
    aembedding = None  # type: ignore

LITELLM_AVAILABLE = aembedding is not None

from src.observability.logging import get_logger
from src.rag.cache import L1RedisCache
...
class EmbeddingCache:
    ...
    def set(self, text: str, model: str, embedding: list[float]) -> None:
        ...
        while len(self._cache) >= self.max_size:
            oldest_key = min(self._access_times, key=lambda key: self._access_times[key])
...
class EmbeddingService:
    ...
    def __init__(self, config: VectorStoreConfig | None = None):
        ...
        self._redis_cache: L1RedisCache | None = None
        if self.config.cache.enabled:
            self._cache = EmbeddingCache(
                max_size=self.config.cache.max_size,
                ttl_seconds=self.config.cache.ttl_seconds,
            )
            self._redis_cache = self._build_redis_cache()

        self.logger = logger

    def _build_redis_cache(self) -> L1RedisCache | None:
        try:
            ...
            self.logger.warning("embedding_redis_cache_init_failed", error=str(e))
            return None
    ...
    async def _get_cached_embedding(self, text: str, model: str) -> list[float] | None:
        ...
        try:
            cached = await self._redis_cache.get_embedding(cache_key, model)
            if cached is not None and self._cache is not None:
                self._cache.set(text, model, cached)
            return cached
        except Exception as e:
            self.logger.warning("embedding_redis_cache_get_failed", error=str(e))
            if self._cache is not None:
    ...
    async def _cache_embedding(self, text: str, model: str, embedding: list[float]) -> None:
        ...
        cache_key = self._make_cache_key(text, model)
        try:
            stored = await self._redis_cache.set_embedding(cache_key, model, embedding)
            if not stored:
                self.logger.warning("embedding_redis_cache_store_skipped")
        except Exception as e:
            self.logger.warning("embedding_redis_cache_set_failed", error=str(e))
    ...
    async def embed_text(
        ...
        if use_cache and self._cache is not None:
            cached = await self._get_cached_embedding(text, self.config.embedding.model)
            if cached is not None:
        ...
        try:
            ...
            if aembedding is None:
                raise EmbeddingProviderError("litellm embedding function is unavailable")
            ...
            # Cache the result
            if use_cache and self._cache is not None:
                await self._cache_embedding(text, self.config.embedding.model, embedding_data)
        ...
        except (EmbeddingDimensionError, EmbeddingProviderError):
            raise
        except Exception as e:
            latency_ms = (time.monotonic() - start_time) * 1000
        ...
        for batch_start in range(0, len(texts_to_embed), batch_size):
            ...
            try:
                ...
                for idx, result in zip(batch_indices, batch_results):
                    ...
                    if use_cache and self._cache is not None:
                        await self._cache_embedding(result.text, result.model, result.embedding)
